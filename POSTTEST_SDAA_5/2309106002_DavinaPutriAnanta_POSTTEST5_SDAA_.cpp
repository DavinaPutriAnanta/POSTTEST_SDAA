...
void merge(Workout* left, Workout* right, Workout** head, bool ascending) {
    Workout* result = nullptr;
    ...
    if ((ascending && left->durasiMenit <= right->durasiMenit) || (!ascending && left->durasiMenit >= right->durasiMenit)) {
        result = left;
        merge(left->next, right, &(result->next), ascending);
    } else {
        result = right;
        merge(left, right->next, &(result->next), ascending);
    }
    *head = result;
}

void split(Workout* source, Workout** front, Workout** back) {
    Workout* fast;
    Workout* slow;
    slow = source;
    fast = source->next;
...
void mergeSort(Workout** headRef, bool ascending = true) {
    ...
    merge(left, right, headRef, ascending);
}

void sortStack(bool ascending) {
    mergeSort(&stackTop, ascending);
    cout << "Stack berhasil diurutkan!" << endl;
}

void sortQueue(bool ascending) {
    mergeSort(&queueFront, ascending);
    cout << "Queue berhasil diurutkan!" << endl;
}

Workout* partition(Workout* low, Workout* high, bool ascending) {
    int pivot = high->durasiMenit;
    Workout* i = low->next;
    Workout* j = low;

    for (Workout* current = low; current != high; current = current->next) {
        if ((ascending && current->durasiMenit <= pivot) || (!ascending && current->durasiMenit >= pivot)) {
            j = (i == nullptr) ? low : i->next;
            swap(current->durasiMenit, i->durasiMenit);
            i = i->next;
        }
    }
    swap(high->durasiMenit, i->durasiMenit);
    return i;
}

void quickSort(Workout* low, Workout* high, bool ascending = true) {
    if (high != nullptr && low != high && low != high->next) {
        Workout* pivot = partition(low, high, ascending);
        quickSort(low, pivot->next, ascending);
        quickSort(pivot->next, high, ascending);
    }
}

void sortQueueDescending() {
    quickSort(queueFront, queueRear, false);
    cout << "Queue berhasil diurutkan secara descending!" << endl;
}

void sortStackDescending() {
    quickSort(stackTop, nullptr, false);
...
void menuStack() {
    ...
    do {
        ...
        cout << "5. Sort Stack Ascending" << endl;
        cout << "6. Sort Stack Descending" << endl;
...
void menuQueue() {
    ...
    do {
        ...
        cout << "5. Sort Queue Ascending" << endl;
        cout << "6. Sort Queue Descending" << endl;
        cout << "7. Kembali ke Menu Utama" << endl;
        cout << "Pilih opsi: ";
        cin >> pilihanQueue;
    ...
    } while (pilihanQueue != 7);
}

int main() {
    int pilihan;
    do {
        cout << "\nProgram Manajemen Workout\n";
